...
function dbConnect($type) {
        ...
        //$conn = new mysqli('localhost', $user, $pwd, 'arrow_main_live')
        $conn = new mysqli('localhost', $user, $pwd, 'arrow_main')
...
#################################################################
 ...
 if(isset($_SESSION['loggedin'])){
	...
	//echo "<h1>member $member_id</h1>";		
	$conn_location = dbConnect('query');
		  
	$sql_location = 'SELECT * FROM member WHERE member_id = '.$member_id.'';
	//echo "<h1>$member_id</h1>";			  
				  //submit the SQL query to the database and get the result
				  $result_location = $conn_location->query($sql_location) or die(mysqli_error());
			    
				    while ($row_location = $result_location->fetch_assoc()) {
				    	//print_r($row_location);

					   	$firstname =	ucwords(strtolower($row_location['member_fname']));
						$lastname =		ucwords(strtolower($row_location['member_lname']));
						$username =		ucwords(strtolower($row_location['username']));
						$email =		ucwords(strtolower($row_location['email']));
						$street =	ucwords(strtolower($row_location['street']));
						$city =		ucwords(strtolower($row_location['city']));
						$state =	ucwords(strtoupper($row_location['state']));
						$zip =		ucwords(strtolower($row_location['zip']));
					}
...
function formatIdentity($num)
{
    ...
    $len = strlen($num);		
    if($len == 9)
        $num = preg_replace('/([0-9]{3})([0-9]{2})([0-9]{4})/', '$1-$2-$3', $num);
    else
        $num = "error";
