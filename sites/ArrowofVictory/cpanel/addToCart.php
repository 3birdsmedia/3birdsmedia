...
echo "key  $key  ------- value   $value  <br />";
    ...
    $bprice = $_POST["bpriceHidden"];
    $quantity = $_POST["qty"];
	$up = $_POST["up"];
    $bquantity = $_POST["bqty"];
	$type = $_POST['enable'];
    ...
    echo "<br/>"; 
     print_r($bprice); //for troubleshooting
    echo "<br/>"; 
    print_r($quantity); //for troubleshooting
    echo "<br/>"; 
    print_r($bquantity); //for troubleshooting
    echo "<br/>"; 
    print_r($item_qty); //for troubleshooting
	echo "<br/>"; 
    print_r($up); //for troubleshooting
	echo "<br/>extra"; 
    print_r($extra); //for troubleshooting
	echo "<br/>"; 
...
if (!isset($_SESSION['cart'])) {
	...
	//loop through each row in POST array and add to _SESSION variable rather than printing it out
		  ...
		  } else if ( is_array($bquantity) && $type == 'pprint') {
	
    
    		  	//now that cart has been created, then loop through items and add to 'cart' session variable
			  	foreach($bquantity as $key => $item_bqty) {
				  	//for security, force the inputs from the form into an integer type using intval()
				  	$item_bqty = intval($item_bqty);
				  	if ($item_bqty > 0) {
							//need to set the product id to a variable so we can get back to our details page 
							//we came from on the header code at end of script
							$id = intval($prod_id[$key]);
							//keeping track of how many items we have
							echo "Total bulk items before adding is: ".  $_SESSION['bitems'] . "</br>";
							//add the new items to the existing number of items in the session
							$_SESSION['bitems'] = $_SESSION['bitems'] + intval($item_bqty);
							echo "Total bulk items after adding is: ".  $_SESSION['bitems'] . " the item_bqty is = $item_bqty</br>";
							//Add the completely new item to the cart session
							array_push($_SESSION['cart'], 
													  array(
															'prod_id' => intval($prod_id[$key]),
...
//IF NO MATCH, THEN ADD ALL ELEMENTS OF NEW ITEM TO SESSION
	...
	echo "<hr>";//for troubleshooting
	echo "Product ID on POST is: " . $prod_id . "<br/>";//for troubleshooting
	echo '<h1>'.$prod_id.'</h1>';
	echo "Type on POST is: " . $type_POST . "<br/>";//for troubleshooting
	 	...
	 	if ($item_qty > 0) {
		  ...
		  //INNER LOOP: LOOPING THROUGH CART FOR THIS PARTICULAR POST ITEM
			 ...
			 //setting the CART product id and type id for use in if conditional
				...
				$type_CART = $_SESSION['cart'][$row]['type'];
									 
			 //for troubleshooting
			 echo "Already in cart:  Product id is $prod_id_CART and type is $type_POST<br/>";
			 echo "On the post: Product id is $prod_id and type is $type_CART<br/>";
			 ...
			 if (($prod_id == $prod_id_CART) && ($type_CART == $type_POST)) {
	...
	//for security, force the inputs from the form into an integer type using intval()
	$item_bqty = intval($item_bqty); //POST QUANTITY
	...
	//create variables for POST items to use in comparison in the CART LOOP below
					...
					echo "TYPE ID on POST is: " . $type_POST . "<br/>";//for troubleshooting
	...
	echo "noMatch is: " . $noMatch . "<br/>"; //for troubleshooting
	 	...
	 	if ($item_bqty > 0) {
		  ...
		  //INNER LOOP: LOOPING THROUGH CART FOR THIS PARTICULAR POST ITEM
			 ...
			 //for troubleshooting
			 echo "Already in cart:  Product id is $prod_id_CART and type is $type_CART<br/>";
			 echo "On the post: Product id is $prod_id_POST and type is $type_POST<br/>";
						 
			  //COMPARE product id on post (outer loop) to the current row in the cart (inner loop)
			 //COMPARE type id on post (outer loop) to the current row in the cart (inner loop)
			 if (($prod_id_POST == $prod_id_CART) && ($type_CART == $type_POST)) {
					 ...
					 echo "noMatch is: " . $noMatch . "<br/>"; //for troubleshooting
										 ...
										 echo "<p>Product and Type on POST match one in the CART!!!</p>"; 
										 echo "Quantity already on session is : " . $_SESSION['cart'][$row]['item_qty'] . "<br/>";
										 echo "Quantity to add is: $item_bqty <br/>";
										 
										 //increment quantity
										 $_SESSION['cart'][$row]['item_qty'] = $_SESSION['cart'][$row]['item_qty'] + $item_bqty;
										 ...
										 //Change Price
										 $_SESSION['cart'][$row]['price'] = $bprice;
										 
										 echo "<h1>".$_SESSION['cart'][$row]['price'] = $bprice."</h1>";
										 
										 
										 //for troubleshooting
										 echo "After increment: NEW Quantity on session is : " . $_SESSION['cart'][$row]['item_qty'] . " and the new price is ". $_SESSION['cart'][$row]['price']." <br/>";
...
if ( is_array($quantity) && $type == 'blank') {
	  ...
	  foreach($bquantity as $key => $item_bqty) {
		  ...
		  if ($item_bqty > 0) { //if any item is more than 0
			$cartCount = true; //setting the $cartCount variable to true, because something was ordered in at least
								//one of the item types of mug, mousepad or poster
			echo "Cart count set to true.";
			break; //only needs to be set once, so break the loop once we find an item that's been ordered
